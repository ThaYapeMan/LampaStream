...
it('preserves the full editor peak Manual grid and stacked Reshape layout', async () => {
  const user = await renderAndOpen()
  await user.click(screen.getByTestId('mode-expert-btn'))
  await user.click(screen.getByRole('button', {name:/^Peak envelope/}))
  const mode = screen.getByRole('group', {name:'Peak envelope mode'})
  const agcRow = mode.parentElement!
  expect(agcRow).toHaveAttribute('class', 'flex flex-wrap items-center gap-2')
  expect(agcRow).toHaveTextContent('Self-calibrating with preset attack and release.')
  await user.click(screen.getByRole('button', {name:'Manual', exact:true}))
  const attack = screen.getByTestId('field-peak-attack')
  const release = screen.getByTestId('field-peak-release')
  const grid = attack.closest('label')!.parentElement!
  expect(grid).toHaveAttribute('class', 'grid grid-cols-2 gap-3')
  expect(grid).toBe(agcRow.nextElementSibling)
  expect(release.closest('label')!.parentElement).toBe(grid)
  expect(agcRow).not.toContainElement(attack)
  expect(agcRow).not.toContainElement(release)
  expect(attack).toHaveValue(.05)
  expect(release).toHaveValue(2)
  const reshapeRow = screen.getByRole('group', {name:'Reshape settings'})
  expect(reshapeRow).toHaveAttribute('class', 'space-y-2 border-t border-border/50 pt-2')
  expect(grid.nextElementSibling).toBe(reshapeRow)
  await user.click(screen.getByRole('checkbox', {name:'Reshape'}))
  expect(screen.getByTestId('field-peak-reshape-power').parentElement)
    .toHaveAttribute('class', 'block space-y-1 text-xs')
  await user.click(screen.getByRole('button', {name:'Auto', exact:true}))
  expect(screen.queryByTestId('field-peak-attack')).not.toBeInTheDocument()
  expect(screen.queryByTestId('field-peak-release')).not.toBeInTheDocument()
