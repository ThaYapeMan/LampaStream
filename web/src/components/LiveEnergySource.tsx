...
export function LiveEnergySource({ profile, active, expertMode = false, onUpdated, onOpen }: {
  ...
  const values = () => ({ floor: String(profile?.lufs_floor ?? -30), ceiling: String(profile?.lufs_ceiling ?? -8), tau: String(profile?.adaptation_tau_s ?? 60), attack: String(profile?.peak_attack_s ?? 0.05), release: String(profile?.peak_release_s ?? 2) })
  ...
  function commit(next = draft) {
    cancelStep()
    if (!profile || validateEnergySettings({ ...next, source: profile.energy_source ?? 'sustained', peakAuto: profile.peak_envelope_auto ?? true })) return
...
function draftKey(field: EnergySetting) {
  return field === 'lufs_floor' ? 'floor' : field === 'lufs_ceiling' ? 'ceiling'
    : field === 'peak_attack_s' ? 'attack' : field === 'peak_release_s' ? 'release' : 'tau'
}
