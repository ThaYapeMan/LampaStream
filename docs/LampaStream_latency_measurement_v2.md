...
LampaStream taps audio at the source. Whatever plays it back adds delay before the
...
a Sonos room and the LampaStream virtual player synced together and playing:

```
SONOS::Study   time: 102.647383
LampaStream        time: 102.648604
```
...
Selection follows from the player: LampaStream reads `sync_master` from the LMS
...
LampaStream should speak UPnP itself rather than asking sonos-squeezebox for the
figure. Two reasons: it keeps LampaStream independent of that project (the same
...
Stored globally, keyed by player MAC. On activation LampaStream reads
